...
if __name__ == "__main__":
    ...
    high2 = 1.
    col_name = 'col'
    df_pilot_group = pd.DataFrame.from_dict({col_name : np.random.uniform(low=low, high=high, size = size)})
    df_control_group = pd.DataFrame.from_dict({col_name : np.random.uniform(low=low2, high=high2, size = size)})
    effect_to_second_type_error = estimate_second_type_error(df_pilot_group, df_control_group,
                            metric_name = col_name, 
                            effects = [1.03, 0.97],
                            alpha=0.05, n_iter=10000)
    print('effect_to_second_type_error', effect_to_second_type_error)